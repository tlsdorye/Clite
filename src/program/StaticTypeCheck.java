	...
	public static void V(Expression e, TypeMap tm) {
		...
		if (e instanceof Unary) {
			...
			if (u.op.NotOp())
				...
				check((type == (Type.INT) || type == (Type.FLOAT)), "type error fot NegateOp" + u.op);
			else
				throw new IllegalArgumentException("shoud never reach here UnaryOp error");
			return;
		}
		throw new IllegalArgumentException("should never reach here");
	}
	...
	public static void V(Statement s, TypeMap tm) {
		...
		// student exercise
			...
			if (testtype == Type.BOOL) {
				...
				check(false, "pooly typed if in Conditional: " + c.test);
			...
			if (testtype == Type.BOOL) {
				...
				check(false, "pooly typed test in while Loop in Conditional: " + l.test);
